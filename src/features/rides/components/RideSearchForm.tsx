...
import type { RideSearchParams } from "../types/ride"
...
type RideSearchFormProps = {
  className?: string
  values?: RideSearchParams
  onValuesChange?: (values: RideSearchParams) => void
  onSubmit: (params: RideSearchParams) => void
}

export function RideSearchForm({ className, values: controlledValues, onValuesChange, onSubmit }: RideSearchFormProps) {
  const [internalValues, setInternalValues] = useState<RideSearchParams>({
    departure: "",
    arrival: "",
    date: "",
  })
  ...
  const patchValues = (patch: Partial<RideSearchParams>) => {
  ...
  return (
    <form
      ...
      aria-label="Recherche de covoiturage"
    >
          ...
          <div className="min-w-0 flex-1">
            <label className="block">
              ...
              <input
                ...
                onChange={(e) => patchValues({ departure: e.target.value })}
          ...
          <div className="min-w-0 flex-1">
            <label className="block">
              ...
              <input
                ...
                onChange={(e) => patchValues({ arrival: e.target.value })}
                placeholder="Ville, adresse…"
                autoComplete="address-level2"
                className={cn(
                  "mt-1 w-full bg-transparent text-[15px] font-medium text-zinc-900 outline-none placeholder:text-zinc-400",
        ...
        <FieldShell className="md:rounded-[26px]">
          ...
          <div className="min-w-0 flex-1">
            <label className="block">
              ...
              <input
                value={values.date}
                onChange={(e) => patchValues({ date: e.target.value })}
                type="date"
                className={cn(
